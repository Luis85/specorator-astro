...
- **Acceptance criteria:**
    - [x] `EnsureProject` is pure and unit-tested (decision logic only); file/process work
          lives in the adapter.
    - [x] `verify:template` (Astro `check` + fixture build of the template) exists, is
          wired into CI, and is green.
    - [x] A build step embeds `templates/astro/**` into `main.js` (generated asset module);
...
- **Acceptance criteria:**
    - [x] Writes one JSON file per snapshot under the project data dir, plus any index the
          loader (C5) needs.
    - [x] Partial failure mid-write never leaves a half-written data dir (temp-dir + swap),
          covered by a test that injects a failure.
    - [x] Uses `normalizePath` for vault-relative paths; writes via Node `fs` only.
...
- **Acceptance criteria:**
    - [x] Value→`CellValue` and grouping mappers are pure and unit-tested in `core/`.
    - [x] Reads view config via `this.config.getOrder()` / `getDisplayName()`; does **not**
          reimplement filters/formulas.
    - [x] Bases-disabled (`registerBasesView` returns false) surfaces a clear error (FR-10).
...
- **Acceptance criteria:**
    - [x] A pure stdout-URL parser lives in `core/` and is unit-tested.
    - [x] Settings expose dev-server port (default 4321) and a Node/binary-path override
          (FR-8); the adapter honors them.
    - [x] Settings gain a **schema version + forward migration** here, as they first expand
...
- **Acceptance criteria:**
    - [x] A fixture table/cards/list snapshot builds to a route with correct
          order/grouping/columns, asserted by `verify:template`.
    - [x] Uses a stable virtual-module/registry barrel so new files dodge Astro's new-file
          HMR gap (D9).
...
- **Acceptance criteria:**
    - [x] Trigger/auto-sync/debounce orchestration is in core use-cases, fake-tested.
    - [x] Disabled Bases / Web Viewer core plugins produce a clear Notice (FR-10).
...
- **Acceptance criteria:**
    - [x] A pure resolver (vault asset ref → site asset URL/path) lives in `core/`, is
          unit-tested, and dedupes/normalizes paths.
    - [x] Referenced images/attachments land in the build output and load from the
          generated pages (asserted by `verify:template` on a fixture).
    - [x] Missing/oversized assets degrade gracefully (logged, not fatal).
...
- **Acceptance criteria:**
    - [x] Detail-route derivation + collision detection across the shared `[...slug]`
          namespace lives in the pure route table (extend `routing.ts`), unit-tested (FR-15).
    - [x] Block refs / transclusions / Dataview are explicitly out of scope and degrade
          gracefully (D8).
...
- **Acceptance criteria:**
    - [x] `theme.css` overrides win over template tokens with no component edits (asserted
...
- **Acceptance criteria:**
    - [x] `BuildSite` orchestration is pure and fake-tested; export-location setting exists
          (FR-8).
...
- **Acceptance criteria:**
    - [x] Pure registry-resolution (user shadows template) + per-(base,view) assignment
...
- **Acceptance criteria:**
    - [x] **Consent is a hard gate (FR-18/D11):** build-time-executing code-fence components
          do nothing until one-time consent is granted; revocable; state persisted.
    - [x] Pure transpile/parse + precedence resolution (user > template, FR-11j) in `core/`,
...
- **Acceptance criteria:**
    - [x] A page-loader adapter reads designated notes; slug/permalink routing + home-page
...
- **Acceptance criteria:**
    - [x] Pure nav-resolution (settings nav list → `navigation` snapshot) in `core/`,
...
- **Acceptance criteria:**
    - [x] Build emits `sitemap.xml` covering static + `[...slug]` routes; `output: 'static'`
          preserved (asserted by `verify:template`).
    - [x] Missing `site` URL warns but does not fail dev; build SEO degrades gracefully.
...
- **Acceptance criteria:**
    - [x] Pure link-resolution (on-site vs off-site against the route table) in `core/`,
